...
import imgFile01 from "../img/ファイル解説/01.PNG" ;
import imgFile02 from "../img/ファイル解説/02.PNG" ;
import imgFile03 from "../img/ファイル解説/03.PNG" ;
import imgFile04 from "../img/ファイル解説/04.PNG" ;
import imgFile05 from "../img/ファイル解説/05.PNG" ;
import imgFile06 from "../img/ファイル解説/06.PNG" ;
import imgFile07 from "../img/ファイル解説/07.PNG" ;
import imgFile08 from "../img/ファイル解説/08.PNG" ;
import imgFile09 from "../img/ファイル解説/09.PNG" ;
import imgFile10 from "../img/ファイル解説/10.PNG" ;
import imgFile11 from "../img/ファイル解説/11.PNG" ;
import imgFile12 from "../img/ファイル解説/12.PNG" ;
import imgFile13 from "../img/ファイル解説/13.PNG" ;
import imgFile14 from "../img/ファイル解説/14.PNG" ;
import imgFile15 from "../img/ファイル解説/15.PNG" ;
    ...
    },
    {
        ...
        "jsx":
            <Section>
                ...
                <h2>ファイルを作成・編集する</h2>
                まずファイルを作成するには<b>実行タブから
                ファイルを管理する</b>を選択します。
                <ImgLine cap="実行タブからファイルを管理する" src={imgFile01} />

                すると、ファイル管理ウィンドウが出てきます。
                <ImgLine cap="ファイルの管理ウィンドウ" src={imgFile02} />
                そこにはファイルの一覧が表示されますが、
                まだ何も作成していないので何も表示されていません。
                新規作成ボタンを押してファイルを作成しましょう。
                <ImgLine cap="新規作成ボタン" src={imgFile03} />

                新規作成ボタンを押すとファイルの編集ウィンドウが表示されます。
                このウィンドウででファイルの詳細を入力していきます。
                <ImgLine cap="ファイル編集ウィンドウ" src={imgFile04}/>
                まずは<b>ファイル名</b>を変更しましょう。今回はテストの点数をファイルで管理することにします。
                なのでファイル名は「<b>テストの点数.csv</b>」(.csvは拡張子)としましょう。
                <NewLine />
                つぎに<b>データ列名</b>を変更しましょう。
                データ列名とはファイルの1行を読み込んだときに
                それぞれのデータにつける名前です。例えば「てべすてん,50」
                というデータを読み取った時に、"てべすてん"という文字に"生徒名"、
                "50"という数字に"点数"という名前を付けておくと、FBEは自動的に
                 生徒名 という変数に"てべすてん"、 点数 という変数に"50"を入れてくれます。
                変数に入れておけば後はそれを表示したり計算に使ったりできます。
                （ここはよくわからなければとりあえず飛ばしてもかまいません）
                今回はファイルの1行1行に「出席番号 , 点数」という形式で
                保存することにします。なので、データ列名には「出席番号,点数」
                と入力します。
                <NewLine />
                ここまで入力すると以下のようになっているはずです。
                <ImgLine cap="" src={imgFile05}/>

                つづいてファイルにデータを記述していきます。
                編集画面中央の白色のテキストエリアに
                次のようなデータを追加しましょう。
                （今回は次のコードをコピーしましょう）
                <div className="code">
                ...
                ここまで入力すると、次のようになるはずです。
                確認出来たら保存ボタンを押すと、保存され、ウィンドウが閉じます。
                <ImgLine cap="" src={imgFile06}/>
                
                ファイルの管理ウィンドウに戻ると今作ったファイルが追加されているのが
                確認できると思います。
                <ImgLine cap="" src={imgFile07}/>
                これでファイルの作成は完了です。
                <h2>ファイルを読み込む</h2>
                フローチャート内でファイルを読み込むには、
                データ記号を使用します。フローチャートにデータ記号を追加しましょう。
                その後、 タイプ オプションにファイル入力を選択しましょう。
                <ImgLine cap="" src={imgFile08}/>
                つぎに対象オプションにはどのファイルから入力したいか、
                ファイル名を指定しましょう。今回は「テストの点数.csv」
                を指定しています。
                <ImgLine cap="" src={imgFile09}/>

                これでファイルから1行を読み込むことが出来ます。
                実行して確認してみましょう。
                きちんと出席番号と点数という変数が読み込まれているのが分かります。
                <ImgLine cap="" src={imgFile10}/>

                データ記号を1つしか書かないとデータは
                1行しか読み込まれません。
                全行をループする形で繰り返すには、ループ記号を使って、
                データがある間繰り返すように設定しましょう。
                まず、ループ記号を追加して、オプションを設定します。
                <b>条件オプションにファイル名</b>,タイプにデータがある間を指定します
                （ファイル名を忘れずに指定してください）。
                <ImgLine cap="" src={imgFile11}/>
                データがある間にデータを読むようにデータ記号を追加しましょう。
                <ImgLine cap="" src={imgFile12}/>
                また、読み込んだデータを「〇〇は××点でした」と表示するように、
                データ記号を追加します。
                <ImgLine cap="" src={imgFile13}/>
                これで簡単なフローチャートが完成しました。
                実行タイプに<b>実行コンソール</b>を選んで、
                実行してみましょう。次のように表示されるはずです。
                <ImgLine cap="" src={imgFile15}/>
                これでデータを繰り返し読むことが出来ました。
